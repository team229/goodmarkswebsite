import type { InternalLink } from '../lib/internalLinks';

export const blogLinks: Record<string, InternalLink[]> = {
  '2-year-iit-jee-program-gurgaon-beats-rushed-crash-course': [
    { kw: '2 year IIT JEE program Gurgaon', href: '/course/2-year-integrated-regular' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: 'IIT JEE foundation course Gurgaon', href: '/courses/foundation' },
  ],
  '2-year-iit-jee-program-in-gurgaon-everything-you-need-to-know-before-you-enroll': [
    { kw: '2 year IIT JEE program Gurgaon', href: '/course/2-year-integrated-regular' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: 'IIT JEE foundation course Gurgaon', href: '/courses/foundation' },
  ],
  'one-year-jee-coaching-gurgaon-plan': [
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: '1 year JEE coaching Gurgaon', href: '/course/1-year-regular-12' },
    { kw: 'JEE dropper course Gurgaon', href: '/courses/iit' },
  ],
  'one-year-jee-coaching-gurgaon-twelve-months-one-shot': [
    { kw: '1 year JEE coaching Gurgaon', href: '/course/1-year-regular-12' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
  ],
  'one-year-for-jee-in-gurgaon-honest-answers-to-the-questions-students-actually-have': [
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: '1 year JEE coaching Gurgaon', href: '/course/1-year-regular-12' },
  ],
  'two-year-iit-jee-program-gurgaon-class-11': [
    { kw: '2 year IIT JEE program Gurgaon', href: '/course/2-year-integrated-regular' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
  ],
  'iit-jee-foundation-course-gurgaon-class-8-10': [
    { kw: 'IIT JEE foundation course Gurgaon', href: '/courses/foundation' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: 'maths coaching for class 10 in Gurgaon', href: '/subject/mathematics' },
  ],
  'iit-jee-foundation-course-gurgaon-class-8-10-batches': [
    { kw: 'IIT JEE foundation course Gurgaon', href: '/courses/foundation' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
  ],
  '1-year-neet-coaching-gurgaon': [
    { kw: '1 year NEET coaching Gurgaon', href: '/course/1-year-regular-12-neet' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
    { kw: 'NEET dropper course Gurgaon', href: '/courses/neet' },
  ],
  'one-year-neet-coaching-gurgaon': [
    { kw: '1 year NEET coaching Gurgaon', href: '/course/1-year-regular-12-neet' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
    { kw: 'NEET dropper course Gurgaon', href: '/courses/neet' },
  ],
  'one-year-neet-coaching-gurgaon-plan': [
    { kw: '1 year NEET coaching Gurgaon', href: '/course/1-year-regular-12-neet' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
  ],
  'best-physics-coaching-for-class-11-near-me': [
    { kw: 'physics tutor for class 11 CBSE Gurgaon', href: '/subject/physics' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
  ],
  'class-12-physics-doesnt-ease-you-in': [
    { kw: 'best physics teacher in Gurgaon for class 12', href: '/subject/physics' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
  ],
  'physics-boards-sneak-up-on-class-10-students': [
    { kw: 'class 10 physics tuition Gurugram board exam', href: '/subject/physics' },
    { kw: 'physics tuition for class 9 Gurgaon', href: '/subject/physics' },
  ],
  'physics-tuition-in-gurgaon-what-to-look-for-and-why-most-students-wait-too-long-to-get-help': [
    { kw: 'physics tutor Sector 85 Gurgaon', href: '/subject/physics' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
  ],
  'physics-tutor-in-sector-85-gurgaon-local-guide': [
    { kw: 'physics tutor Sector 85 Gurgaon', href: '/subject/physics' },
    { kw: 'best physics teacher in Gurgaon for class 12', href: '/subject/physics' },
  ],
  'chemistry-boards-stressing-gurgaon-students': [
    { kw: 'CBSE chemistry tuition class 12 Gurgaon', href: '/subject/chemistry' },
    { kw: 'chemistry tuition for board exams Gurgaon', href: '/subject/chemistry' },
  ],
  'class-11-chemistry-tougher-than-expected': [
    { kw: 'chemistry coaching for class 11 Gurgaon', href: '/subject/chemistry' },
    { kw: 'chemistry tuition classes in Gurgaon', href: '/subject/chemistry' },
  ],
  'class-10-maths-harder-than-it-should': [
    { kw: 'maths coaching for class 10 in Gurgaon', href: '/subject/mathematics' },
    { kw: 'ICSE maths tuition Gurgaon class 10', href: '/subject/mathematics' },
  ],
  'class-11-maths-catches-students-off-guard': [
    { kw: 'maths coaching for class 11 in Gurgaon', href: '/subject/mathematics' },
    { kw: 'IIT JEE coaching in Gurgaon', href: '/courses/iit' },
  ],
  'maths-tuition-for-weak-students-gurgaon': [
    { kw: 'maths tuition for weak students Gurgaon', href: '/subject/mathematics' },
    { kw: 'maths coaching for class 11 in Gurgaon', href: '/subject/mathematics' },
  ],
  'biology-tutor-class-10-gurgaon-boards': [
    { kw: 'biology tutor class 10 Gurgaon board exam', href: '/subject/biology' },
    { kw: 'biology tuition for class 12 Gurgaon', href: '/subject/biology' },
  ],
  'struggling-with-class-10-biology-before-boards': [
    { kw: 'biology tutor class 10 Gurgaon board exam', href: '/subject/biology' },
    { kw: 'CBSE biology tuition class 12 Gurugram', href: '/subject/biology' },
  ],
  'why-class-11-biology-feels-like-a-different-subject': [
    { kw: 'biology ncert solutions coaching class 11', href: '/subject/biology' },
    { kw: 'biology coaching near Sohna Road Gurugram', href: '/subject/biology' },
    { kw: 'NEET coaching in Gurgaon', href: '/courses/neet' },
  ],
  'beyond-the-textbook-why-gurgaon-parents-are-rethinking-cbse-tuitions': [
    { kw: 'maths tuition for weak students Gurgaon', href: '/subject/mathematics' },
    { kw: 'CBSE chemistry tuition class 12 Gurgaon', href: '/subject/chemistry' },
    { kw: 'CBSE biology tuition class 12 Gurugram', href: '/subject/biology' },
  ],
  'manesars-quiet-coaching-problem-why-board-prep-and-entrance-exam-prep-keep-getting-treated-as-two-different-jobs': [
    { kw: 'maths coaching for class 10 in Gurgaon', href: '/subject/mathematics' },
    { kw: 'CBSE chemistry tuition class 12 Gurgaon', href: '/subject/chemistry' },
    { kw: 'CBSE biology tuition class 12 Gurugram', href: '/subject/biology' },
  ],
};